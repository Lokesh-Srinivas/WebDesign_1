...
var nameAndId = "Lokesh Srinivas - 002038702";
...
function showAndHideDropDownTextArea(dropdown) {
    ...
    // Check if the nextElement exists and has the expected class
    if (!nextElement || !nextElement.classList.contains("dropDownTextArea")) {
        console.error("Dropdown row not found for the selected student.");
        return;
    }

    // Toggle visibility
    nextElement.style.display = (nextElement.style.display === "none" || nextElement.style.display === "") ? "table-row" : "none";
}
